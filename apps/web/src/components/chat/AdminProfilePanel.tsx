    ...
    <aside className="flex h-full w-full shrink-0 flex-col border-r border-[var(--border)] bg-[var(--surface-raised)] sm:w-[clamp(16rem,24vw,22rem)]">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-6">
        <div className="text-center">
          {site.avatarUrl ? (
            <img src={site.avatarUrl} alt={site.displayName} className="mx-auto h-24 w-24 rounded-full object-cover" />
          ) : (
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              {site.profilePhotos.map((photo, index) => (
                <img
                  ...
                  src={photo}
                  alt={`${site.displayName} profile photo ${index + 1}`}
