...
The public site profile includes the admin avatar, bio, contact links, PGP public key, and optional media gallery. Images and animated GIFs render as native images, while videos require an explicit play action and support byte-range requests for seeking. New gallery media lives in the configured local or S3-compatible storage adapter; the migration preserves older database-embedded photos as immutable legacy media. The browser tab title is configurable, while the favicon is derived only from the admin avatar so there is no second logo source to drift out of sync.
