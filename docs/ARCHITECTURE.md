...
**Link previews are the one exception, and they are opt-out.** YouTube/Vimeo embeds (`VideoEmbed.tsx`) are pure client-side URL pattern matching - no server involvement, no exception to anything. Every other link gets an Open Graph preview card, which the server fetches on the client's behalf (`GET /api/link-preview`, `apps/server/src/services/linkPreview/`) - the browser can't do this itself for the vast majority of sites (CORS blocks a cross-origin fetch of arbitrary HTML), so a same-origin server round-trip is the only technically workable approach, not just the more convenient one. The server learns _that a URL was shared_ (and, since previews are fetched per-message-render rather than cached against the ciphertext, only while a client renders that message) - it never learns the message it was shared in, and this happens for a message actually decrypted by the requesting client, not by inspecting any ciphertext. This is a real, if narrow, expansion of server-side metadata beyond the existing "who talked to whom, when, how much" - flagged here rather than left implicit. An operator who would rather the server never originate outbound requests on a user's behalf can disable link previews in System settings.
...
The public site profile includes the admin avatar, bio, contact links, PGP public key, and optional media gallery. Images and animated GIFs render as native images. Videos remain inert thumbnails in both profile grids and open a shared full-screen player only after an explicit action; byte-range requests support playback and seeking. Admin drag and drop ordering writes a validated, complete sequence of media positions in one database transaction, and the public profile uses that persisted order. New gallery media lives in the configured local or S3-compatible storage adapter; the migration preserves older database-embedded photos as immutable legacy media. The browser tab title is configurable, while the favicon is derived only from the admin avatar so there is no second logo source to drift out of sync.
...
The admin dashboard separately controls whether a consented IP may be retained and whether a coarse lookup through ipwho.is is allowed. Both are disabled by default. Each row receives an expiry, an hourly cleanup removes expired rows, visitors can revoke their row immediately, and switching the feature off deletes all rows.

Upload limits and other request policies follow the same model. The dashboard stores a global attachment ceiling, per-type client ceilings, files per message, rate limits, link-preview behavior, and notification timing in PostgreSQL. Because attachment metadata is encrypted, the server enforces the global byte ceiling while the sending browser applies the more specific image, video, audio, document, and other-file limits before encryption. A fixed absolute server ceiling remains in code as a resource-safety boundary and cannot be raised from the dashboard.
